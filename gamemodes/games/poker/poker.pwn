...
Poker_ReturnPlayerTable ( playerid ) {
...
Poker_ReturnPlayerHandle(playerid) {
...
Init_LoadPokerTables() {
	...
	mysql_tquery_inline(mysql,"SELECT * FROM poker",using inline Load_PokerTables,"");
	return true;
}
		...
		if(!PlayerPokerData[i][E_PLAYER_TOTAL_CHIPS])
		{
			...
			else
			{
				inline Poker_DialogBuyIn(pid, dialogid, response, listitem, string:inputtext[]) {
   				...
   				Dialog_ShowCallback ( i, using inline Poker_DialogBuyIn, DIALOG_STYLE_MSGBOX, "Buy_In", ""COL_WHITE"You've ran out of chips. Do you want to pay the buy-in fee again to continue playing?", "Yes", "No" );
			}
		}
	}
	//Iter_Clear(IT_PlayersInGame<handle>);
